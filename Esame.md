...
# 2. Obiettivo
Analizzare e quantificare le variazioni spettrali indotte dall'incendio del 2017 sulla copertura vegetale nel Parco Nazionale della Maiella, confrontando le immagini satellitari Sentinel-2 prima e dopo l'evento.
...
```
> **Commento:**
> Dopo l'incendio la vegetazione (in rosso) è ridotta sul versante ovest del monte Morrone, maggiormente colpito dal rogo.
<p align="center">
...
```
In seguito è stata calcolata la variazione multitemporale, $dNDVI = NDVI_{pre} - NDVI_{post}$, per mappare con precisione il perimetro del suolo bruciato e classificare i livelli di danno subiti dall'ecosistema forestale:
```r
...
Per la classificazione *non supervisionata* è stata utilizzata la funzione `im.classify()` del pacchetto `imageRy, che suddivide i pixel in tre gruppi coerenti dal punto di vista spettrale (`num_clusters = 3`):
```r
ndvi_pre_c <- im.classify(ndvi_pre, num_clusters = 3, seed = 1)  # suddivisione dei pixel in tre gruppi (clusters)
